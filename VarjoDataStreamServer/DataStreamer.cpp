...
void DataStreamer::startDataStream(varjo_StreamType streamType, varjo_TextureFormat streamFormat, varjo_ChannelFlag channels)
{
    ...
    if (streamId == varjo_InvalidId) {
        ...
        if (streamId != varjo_InvalidId) {
            ...
            stream.snapshotRequested = false;
...
void DataStreamer::onDataStreamFrame(const varjo_StreamFrame* frame, varjo_Session* session)
{
    ...
    for (varjo_ChannelIndex channelIndex : {varjo_ChannelIndex_Left, varjo_ChannelIndex_Right}) {
        ...
        if (frame->dataFlags & varjo_DataFlag_Intrinsics) {
            intrinsics = varjo_GetCameraIntrinsics2(session, frame->id, frame->frameNumber, channelIndex);
            CHECK_VARJO_ERR(m_session);
        }
...
bool DataStreamer::convertDistortedYUVToRectifiedRGBAandSave(const varjo_BufferMetadata& buffer, const uint8_t* input, const glm::ivec2& outputSize, uint8_t* output,
    const varjo_Matrix& extrinsics, const varjo_CameraIntrinsics2& intrinsics, std::optional<const varjo_Matrix> projection)
{
    std::cout << "hello convertDistortedYUVToRectifiedRGBAandSave\n";
    ...
    writeBMP("distort_vst_aftProc.bmp", buffer.width, buffer.height, 4, outPtr);

    return true;
}

bool DataStreamer::convertDistortedYUVToRectifiedYUV(const varjo_BufferMetadata& buffer, const uint8_t* input, const glm::ivec2& outputSize, uint8_t* output, const varjo_Matrix& extrinsics, const varjo_CameraIntrinsics2& intrinsics, std::optional<const varjo_Matrix> projection)
{
    std::cout << "hello convertDistortedYUVToRectifiedYUV\n";
    if (!(buffer.format == varjo_TextureFormat_NV12)) {
        CRITICAL("Unsupported pixel format: %d", static_cast<int>(buffer.format));
        return false;
    }
    ...
    uint8_t* outPtr = output;
    size_t outOffs = 0;
    uint8_t* out_yStart = outPtr;
    uint8_t* out_uvStart = outPtr + uvOffs;
    for (int y = 0; y < outputSize.y; ++y) {
        for (int x = 0; x < outputSize.x; ++x) {
            // Init to black which will be written in case the sample coordinate is invalid.
            uint8_t R = 0;
            ...
            if (sampleCoord.x >= 0 && sampleCoord.x < inputSize.x && sampleCoord.y >= 0 && sampleCoord.y < inputSize.y) {
                ...
                out_yStart[sampleCoord.y * bufferRowStride + sampleCoord.x] = Y;
                out_uvStart[sampleCoord.y / chromaResDivider * bufferRowStride + uvX + 0] = U;
                out_uvStart[sampleCoord.y / chromaResDivider * bufferRowStride + uvX + 1] = V;
            }
